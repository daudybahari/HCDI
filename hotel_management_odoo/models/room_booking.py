...
class RoomBooking(models.Model):
        ...
        for rec in self:
                ...
                for room in room_lines:
                    ...
                    if booking_dict not in account_move_line:
                        if not account_move_line:
                            ...
                            for am_line in account_move_line:
                                if am_line.get('product_type') == 'room':
                                    if booking_dict['name'] == am_line['name'] and \
                                            booking_dict['price_unit'] == am_line['price_unit'] and \
                                            booking_dict['quantity'] != am_line['quantity']:
                                        booking_list.append(
                                            {'name': room.room_id.name,
                                             "quantity": booking_dict['quantity'] - am_line['quantity'],
                                             "price_unit": room.price_unit,
                                             "product_type": 'room'})
                                    else:
                                        booking_list.append(booking_dict)
            ...
            if food_lines:
                for food in food_lines:
                    booking_list.append(rec.create_list(food))
            ...
            if service_lines:
                for service in service_lines:
                    booking_list.append(rec.create_list(service))
            ...
            if fleet_lines:
                for fleet in fleet_lines:
                    booking_list.append(rec.create_list(fleet))
            ...
            if event_lines:
                for event in event_lines:
                    booking_list.append(rec.create_list(event))
    ...
    def get_details(self):
        ...
        staff = self.env['res.users'].search_count([('groups_id', 'in', group_ids)]) if group_ids else 0
